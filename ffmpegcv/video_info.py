...
def release_process_og(process: Popen, forcekill=False):
    if forcekill:
        print(f"Terminating")
        process.terminate()
    ...
    #     print(f"Waiting to kill process")
    #     process.wait()
...
def release_process(process: subprocess.Popen, forcekill=False):
    if not process:
        return
    ...
    try:
        if forcekill:
            ...
            try:
                ...
                print(f"Process did not terminate in time. Forcefully killing it.")
                process.kill()
                process.wait(timeout=5)
        else:
            process.wait(timeout=5)
        ...
        print(f"Error while releasing process: {e}")
